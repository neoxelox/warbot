...
const gameStart = require("../game_logic/start.js");

async function join(message, args, parties, client, frame) {
    if(args[1] != undefined) {
       try {
            parties.find({$or: [{ id: parseInt(args[1]) }, { name: args[1].replace(/-|_/g, " ") }]}, async (err, docs) => {
                if(docs.length == 1) {
                                ...
                                if(!findByID(docs[0].players, message.author.id)) {
                                    ...
                                    parties.update({_id: docs[0]._id}, { $push: { players: newPlayer } }, {}, (err) => {
                                        if(!err) {
                                            ...
                                            if(docs[0].players.length + 1 == docs[0].slots) gameStart(frame, client, docs[0]._id, parties);
                                        }                                    
                                    });
                                } else {
                    ...
                    } else if(docs[0].status === "STARTED") message.channel.send("**This party has already started!** 😅");
                      else message.channel.send("**This party has already finished!** 😅");
                } 
